 ...
 * 注册 .st show 命令
 */
export function registerStShowCommand(parent: Command, ctx: Context, diceAdapter: DiceAdapter) {
    ...
    .action(async ({ session }, args) => {
      ...
      try {
        ...
        if (attributes.length === 1 && attributes[0].toLowerCase() === 'all') {
          const attrs = await characterService.getAttributes(session, cardName || null);
        ...
        if (attributes.length > 0) {
          const allAttrs = await characterService.getAttributes(session, cardName || null);
