 ...
 * 删除先攻列表
 */
async function deleteInitiative(ctx: Context, channelId: string, platform: string): Promise<void> {
  try {
...
export function registerInitiativeCommands(
  ...
  // .init.del - 移除先攻条目
  init.subcommand('.del <name:text>', '移除先攻条目').action(async ({ session }, name) => {
    if (!session) {
      return '无法获取会话信息';
    }
