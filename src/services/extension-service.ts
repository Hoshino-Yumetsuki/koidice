...
import type { DescriptorJson, ReplyConfig, LoadedPlugin } from './extension/types';
...
import { parseTemplate, createAliasMap, type CharacterTemplate } from './extension/template-parser';
...
export class ExtensionService {
  ...
  async initialize(): Promise<void> {
    ...
    for (const plugin of ordered) {
      if (plugin.descriptor.require?.some((dependency) => !this.loadedPlugins.has(dependency))) {
        logger.error(`Skipping plugin ${plugin.descriptor.name}: a dependency failed to load`);
        continue;
      }
  ...
  private async readDescriptor(pluginPath: string): Promise<DescriptorJson> {
    ...
    if (
      ...
      (!Array.isArray(descriptor.require) ||
        descriptor.require.some((dependency) => typeof dependency !== 'string' || !dependency))
    ) {
    ...
    if (typeof descriptor.dice_build === 'number' && descriptor.dice_build > SUPPORTED_DICE_BUILD) {
      throw new Error(
  ...
  private resolveLoadOrder(plugins: Map<string, DiscoveredPlugin>): DiscoveredPlugin[] {
    const valid = new Map(plugins);
    for (const [name, plugin] of plugins) {
      const missing = plugin.descriptor.require?.filter((dependency) => !plugins.has(dependency));
      if (missing?.length) {
        logger.error(`Invalid plugin ${name}: missing dependencies ${missing.sort().join(', ')}`);
        valid.delete(name);
      }
    }
    ...
    while (changed) {
      ...
      for (const [name, plugin] of valid) {
        const unavailable = plugin.descriptor.require?.find((dependency) => !valid.has(dependency));
        if (unavailable) {
          logger.error(`Invalid plugin ${name}: dependency ${unavailable} is unavailable`);
          valid.delete(name);
          changed = true;
        }
      }
    ...
    while (remaining.size) {
      const ready = [...remaining]
        ...
        .sort();
      if (!ready.length) {
        logger.error(`Invalid plugin dependency cycle: ${[...remaining].sort().join(', ')}`);
        break;
      }
  ...
  async loadPlugin(pluginPath: string, preparedDescriptor?: DescriptorJson): Promise<boolean> {
    ...
    try {
      ...
      try {
        await this.loadScriptsRecursive(scriptDir, scriptDir, descriptor.name, scripts);
      } catch (error) {
  ...
  private async registerScripts(plugin: LoadedPlugin): Promise<void> {
    for (const [scriptName, code] of plugin.scripts) {
      ...
      const processedCode = isLua ? wrapLuaScript(code, plugin.descriptor) : code;
      for (const name of names) {
        const success = isLua
  ...
  private async loadTemplates(plugin: LoadedPlugin): Promise<void> {
    ...
    for (const file of files) {
      const template = parseTemplate(await fs.readFile(path.join(modelDir, file), 'utf-8'));
  ...
  private async loadRules(plugin: LoadedPlugin): Promise<void> {
    ...
    try {
      await loadRulebooks(path.join(plugin.path, 'rulebook'), plugin.name, rules);
    } catch (error) {
      if (this.isMissingFile(error)) return;
      throw error;
    }
  ...
  private async loadScriptsRecursive(
    ...
    for (const entry of entries) {
      ...
      } else if (entry.isFile() && (entry.name.endsWith('.lua') || entry.name.endsWith('.js'))) {
  ...
  private async loadReplyConfigs(
    ...
    const files = (await fs.readdir(replyDir)).filter((file) => file.endsWith('.toml')).sort();
    for (const file of files) {
      const config = toml.parse(await fs.readFile(path.join(replyDir, file), 'utf-8')) as Record<
        string,
        unknown
      >;
      ...
      for (const commandName of Object.keys(replies).sort()) {
        commands.set(commandName, (replies as Record<string, ReplyConfig>)[commandName]);
      }
    }
  }
  ...
  unloadPlugin(name: string): boolean {
    ...
    if (dependents.length) {
      logger.warn(`Cannot unload plugin ${name}; required by ${dependents.sort().join(', ')}`);
      return false;
    }
    this.releasePlugin(plugin);
        ...
        .filter(
          (name) =>
            ![...remaining].some((candidate) =>
              this.loadedPlugins.get(candidate)?.descriptor.require?.includes(name)
            )
        )
        .sort();
