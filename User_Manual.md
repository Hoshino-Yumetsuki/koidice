...
`.game rou X*Y` 轮盘骰是将X面骰（如百面骰）实例化为一个俄罗斯轮盘，直到重置轮盘前，每个已经掷出的点数必出且仅出现Y次。_骰池不会触发轮盘骰机制，因为这将使其失去随机性。_
...
- `.setcoc clr` 清除房规（调用骰娘设置的默认规则）
  ...
  _房规2与3的区别在于：初始值为1的技能在②下大成功率只有1，而在③下有5_
...
该指令可以在群内自设牌堆，使用.draw时，牌堆实例优先级高于同名公共对象。从牌堆实例抽牌不会放回直到抽空。_每个群的牌堆列表至多保存10个牌堆_
...
### .rsr 暗影狂奔(_Shadowrun_)

`.rsr[骰数]`
出5/6视为命中
出1过半视为失误

### .rnc 永夜后日谈(_永い后日谈のネクロニカ_)
...
### .rbh 山屋惊魂(_Betrayal at house on the hill_, 小黑屋)
